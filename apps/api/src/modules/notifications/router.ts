...
export const notificationsRouter = {
  ...
  markRead: os.notifications.markRead
    .use(withUser)
    .handler(({ context, input }) => context.deps.services.notifications.markRead(context.user.id, input.id)),
  ...
  remove: os.notifications.remove
    .use(withUser)
    .handler(({ context, input }) => context.deps.services.notifications.remove(context.user.id, input.id)),
};
